...
from obstacle_course import Tetrominos
import random

class Vehicle:
    def __init__(self,vec_type = 'car'):
        if vec_type == 'car':
...
class Case:
    ...
    def read():
        ...
        case.x0, case.y0, case.theta0 = 10,10,0
        case.xf, case.yf, case.thetaf = 240,240,0
        case.xmin = 0
        case.xmax = 250
        case.ymin = 0
        case.ymax = 250
        # create the terominos
        #Creates an obstacle field based on the input percentage of desired coverage
        grid = [[0] * case.xmax for _ in range(case.ymax)]
        
        # Calculate the total number of cells in the grid
        total_cells = case.xmax * case.ymax

        # Calculate the target number of occupied cells based on the desired percentage
        target_percentage = .1
        target_occupied_cells = int(total_cells * (target_percentage)/25)
        ...
        while occupied_cells < target_occupied_cells and failed_counter < 10000:
            ...
            random_x = random.randint(0, case.xmax - 1 - max_x)
            random_y = random.randint(0, case.ymax - 1 - max_y)
            ...
            for coord in rotated_tetromino:
                ...
                if new_x < 0 or new_x >= case.xmax or new_y < 0 or new_y >= case.ymax:
                    can_place = False
                    break
                if grid[new_y][new_x] == 1:
                    can_place = False
                    break
                obj.append([new_x,new_y])
            # If the Tetromino can be placed, add it to the grid and update the occupied cells count
            if can_place:
                
                case.obs.append(np.array(obj))
...
class Obstacles():
    def __init__(self, points,centerpoint, name) -> None:
        self.name = name
        self.points = points
    ...
    def extinguish_fire(self):
        self.self.isOnFire = True


class Map:
    def __init__(self,
                 discrete_size: np.float64 = 0.1,
                 file: string = None, vehicle: Vehicle = Vehicle()) -> None:
    ...
    def detect_obstacle_edge(self):
        # just consider the boundary of the obstacle
        self.discrete_map()
        ...
        for i in range(0, self.case.obs_num):
            ...
            # Create list of obstacles for determining if obstacles can be lit on fire
            self.obstacles.append(Obstacles(points=old_obstacle, centerpoint=(center_x, center_y), name=str(i)))
