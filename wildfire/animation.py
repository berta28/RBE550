...
class ploter:
    ...
    def plot_obstacles(map:Map, fig_id=None):
        ...
        for obstacle in map.obstacles:
            if obstacle.isOnFire:
                color = 'orange'
            elif not obstacle.isOnFire and not obstacle.canBeLit:
                color = 'black'
            else:
                color = 'green'
            
            plt.fill(obstacle.points[:, 0], obstacle.points[:, 1], facecolor = color, alpha = 1)
