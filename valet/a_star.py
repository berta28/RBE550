...
import math
import queue
from costmap import Map, Vehicle
import collision_check
from compute_h import Dijkstra
import rs_curve
...
class hybrid_a_star:
    ...
    def expand_node_car(self,
        ...
        for i in range(next_index):
            ...
            travel_distance = speed * self.dt
            theta_ = current_node.theta + \
                (self.vehicle.max_v * np.tan(steering_angle)) / \
                self.vehicle.lw * self.dt
            theta_ = rs_curve.pi_2_pi(theta_)
            x_ = current_node.x + travel_distance * np.cos(theta_)
            y_ = current_node.y + travel_distance * np.sin(theta_)

            # Check if node is already open or closed
            ...
            if find_opennode == False:
                ...
                for i in range(math.ceil(self.dt / self.ddt)):
                    ...
                    # i : 0-9
                    travel_distance_i = speed * self.ddt * (i+1)
                    theta_i = current_node.theta + \
                        (self.vehicle.max_v * np.tan(steering_angle)) / \
                        self.vehicle.lw * self.ddt * (i+1)
                    theta_i = rs_curve.pi_2_pi(theta_i)
                    x_i = current_node.x + travel_distance_i * np.cos(theta_i)
                    y_i = current_node.y + travel_distance_i * np.sin(theta_i)
    ...
    def expand_node_truck(self,
        ...
        for i in range(next_index):
            ...
            theta_ = current_node.theta + \
                ...
                self.vehicle.lw * self.dt
            theta_ = rs_curve.pi_2_pi(theta_)
            theta_1 = current_node.theta + current_node.theta_1 + self.vehicle.max_v/self.vehicle.d1 * np.sin(theta_ - current_node.theta_1)*self.dt
            theta_1 = rs_curve.pi_2_pi(theta_1)
            x_ = current_node.x + travel_distance * np.cos(theta_)
            y_ = current_node.y + travel_distance * np.sin(theta_)
            ...
            if find_opennode == False:
                ...
                for i in range(math.ceil(self.dt / self.ddt)):
                    ...
                    theta_i = current_node.theta + \
                        ...
                        self.vehicle.lw * self.ddt * (i+1)
                    theta_i = rs_curve.pi_2_pi(theta_i)
                    theta_i_1 = current_node.theta_1 + self.vehicle.max_v/self.vehicle.d1 * np.sin(theta_ - current_node.theta_1)* np.sin(theta_ - current_node.theta_1)*self.ddt * (i+1)
                    theta_i_1 = rs_curve.pi_2_pi(theta_1)
                    x_i = current_node.x + travel_distance_i * np.cos(theta_i)
                    y_i = current_node.y + travel_distance_i * np.sin(theta_i)
            ...
            # if this node has been explored update cost if needed
            else:
                new_h = self.calc_node_heuristic(child_node)
                new_g = self.calc_node_cost(
                    child_node, father_theta=current_node.theta, father_gear=current_node.forward)
    ...
    def try_rs_curve(self, current_node: Node):
        ...
        for i in range(len(rs_path.x)):
            ...
            path_theta_1 = path_theta+ current_node.theta_1 + self.vehicle.max_v/5.0 * np.cos(path_theta - current_node.theta_1) * np.sin(path_theta - current_node.theta_1)*self.dt
            rs_path.theta_1.append(rs_curve.pi_2_pi(path_theta_1))
            collision = self.collision_checker.check(
    ...
    def finish_path(self, current_node: Node):
        node = current_node
        all_path_node = []
        # create path from curretn node to start node
        while node.index != 0:
        ...
        all_path = [[node.x, node.y, node.theta, node.theta_1]]
        # smooth path out using more waypoints
        for i in range(len(all_path_node)):
            ...
            for j in range(math.ceil(self.dt/self.ddt)):
                ...
                theta_j_1 = theta_j + current_node.theta_1 + self.vehicle.max_v/5.0 *np.sin(theta_j - current_node.theta_1)*self.ddt * (j+1)
                theta_j_1 = rs_curve.pi_2_pi(theta_j_1)
                x_j = all_path_node[k].x + td_j * np.cos(theta_j)
                y_j = all_path_node[k].y + td_j * np.sin(theta_j)
                all_path.append([x_j, y_j, theta_j ,theta_j_1])
